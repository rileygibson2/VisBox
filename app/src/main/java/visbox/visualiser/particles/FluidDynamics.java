...
public class FluidDynamics extends ParticleVisualiser {
    ...
    private float smoothingRadius = 0.05f;
    ...
    public List<List<List<Integer>>> cells;
    ...
    private void setup() {
        ...
        List<List<List<Integer>>> cells = new ArrayList<>();
        ...
        for (int p=0; p<numParticles; p++) {
            ...
            for (int dY=-1; dY<=1; dY++) {
                for (int dX=-1; dX<=1; dX++) {
                    int nGX = gX+dX;
                    int nGY = gY+dY;
                    List<Integer> cell = cells.get(nGY).get(nGX);
    ...
    private float densityKernel(float h, float r) { //Poly6
        if (r>=h) return 0.0f;
